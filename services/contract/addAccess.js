...
module.exports = async function(req, reply) {
    ...
    try {
        ...
        if (contractsResult[0].contract_type == "user" || data.contract_type == "user") {
            ...
            // Add permission accesses
            for (const [index, val] of Object.entries(data["permissions"])) {
                await contractData.addAccess(index, val["attribute"], val["start"], val["end"]);
            }
            // console.log(contractData);

            // No error here means success in adding
            return reply.send({
                success: true,
                msg: "Successfully added access(es)"
            })
        } else if (contractsResult[0].contract_type == "owner" || data.contract_type == "owner") {
            // Owner has all access, nothing to add
            return reply.send({
                success: true,
